...
class _NewArrivalsPageState extends State<NewArrivalsPage> {
  ...
  void initState() {
    super.initState();
    futureBooks = ApiService().fetchNewBooks();
  }

  void searchBooks() async {
    setState(() {
      futureBooks = ApiService().fetchNewBooks(name: searchQuery);
    });
  }
  ...
  void toggleFavorite(int bookId) async {
    ...
    setState(() {
      futureBooks = ApiService().fetchNewBooks(name: searchQuery); // Обновление книг
    });
  }
